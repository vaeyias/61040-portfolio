...
2. **Fixing an action**: addItem action could potentially break this important invariant if the count is allowed to be negative. The problem might be fixed by requiring the count to be positive.
...
3. There are no two users with the same username. It is preserved by not allowing a User to be created if a username already exists.
...
**actions**
    ...
    register (username: String, password: String; email: String): (user: PendingUser)
        requires username to not already exist
        effects creates a new PendingUser with given username and password and a unique secret token. An email with a secret token is sent to the given email and confirmed Flag to set to False.
    ...
    confirm (user: PendingUser, token: String): (user: User)
        ...
        effects if token matches, completes registration of new user by creating a User with given username and password.
...
**principle** when a personal access token is generated, a User can use it to authenticate themselves and grant themselves a certain scope of access. When a personal access token is deleted, the token can no longer be used.
...
**actions**
    ...
    generateToken (user: User, expireDate: String, scope: [String]): (token: PersonalAccessTokens)
        requires username to not already exist, expireDate to be after the present date.
    ...
    authenticate (user: User, token: String, scope:String): (user: User)
        requires user exists, given token is associated with the given user, the token is active, and the scope is in the token's list of scopes it can access.
...
2. Tokens can be created and deleted anytime.
...
**principle** when a shortened URL is created using a full URL and optionally a URL suffix, users can navigate to the website by using the shortened URL with the URL suffix instead of the full URL.
...
**actions**
    ...
    generateRandomShortURL (url: String): (shortURL: ShortenedURL)
        ...
        effects created a ShortenedURL with full URL as the given url and a randomly generated suffix as the URL suffix

    createShortenedURL (url: String, suffix: String): (shortURL: ShortenedURL)
        requires given url is a valid url and suffix is None or suffix is unique and does not exist as a URL suffix among the existing ShortenedURLs
        effects creates a ShortenedURL with full URL as the given url. If suffix is None, then randomly generate a suffix as the URL suffix. If a valid suffix is given, add the suffix

    redirect (suffix: String):
        requires: the suffix exists among the ShortenedURLs.
...
**purpose** allow users to reserve conference rooms in a company or university at certain times.

**principle** When someone books a conference room for a certain hour, a reservation is created and no one else can reserve the same room at the same hour. When someone no longer needs to use the room they reserved at a certain time, they can retract their reservation and other users can now book the same room at that hour.

**state**

    a set of Rooms with
        a room number
        a set of BookingHours

    a set of BookingHours with
        a time
        an available Flag
...
**actions**
    ...
    reserve (user: User, booking: BookingHour): (reservation: Reservation)
        requires user to exist and booking to exist and to be available.
        effects creates a Reservation with given user as the host and the booking as the BookingHour. Sets the available Flag for the booking to unavailable..

    unreserve (reservation: Reservation): (reservation: Reservation)
        requires reservation to exist
        effects deletes the reservation and sets the available Flah of the BookingHour under the reservation to available.
Notes:
- booking times are assumed to be every hour.
...
**concept** BillableHoursTracking [Client, Employee, Time]
...
**actions**
    ...
    startSession(project: Project, employee: User, startTime: Time, defaultEndTime: Time, description: String) : (session: Session)
